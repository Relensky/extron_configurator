...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'screen get-info: '), self.__MatchQuery, None)
            self.AddMatchString(re.compile(b'login: '), self.__MatchLogin, None)
            self.AddMatchString(re.compile(b'Password: '), self.__MatchPassword, None)
            self.AddMatchString(re.compile(b'> '), self.__MatchConnected, None)
    ...
    def SetScreen(self, value, qualifier):
        ...
        SetScreenNames = {
            ...
            'Stop': 'screen move stop\n'
        }

        if value in SetScreenNames:
            CommandString = SetScreenNames[value]
        else:
            self.Discard('Invalid Command for SetScreen')

        self.__SetHelper('Screen', CommandString, value, qualifier)

    def __MatchConnected(self, match, qualifier):
        self.OnConnected()
    ...
    def SetLogin(self, value, qualifier):
        #print('Login Sent {}'.format(self.deviceUsername))
    ...
    def SetPassword(self, value, qualifier):
        #print('Password Sent {}'.format(self.devicePassword))
        if self.devicePassword is not None:
            self.Send(self.devicePassword + '\n')
        else:
            self.Discard('Password is missing')

    def UpdateQuery(self, value, qualifier):

        CommandString = 'screen get-info' + '\n'
        self.__UpdateHelper('Query', CommandString, value, qualifier)

    def __MatchQuery(self, match, tag):

        self.WriteStatus('Query', match.group())

    def __SetHelper(self, command, commandstring, value, qualifier):
        self.Debug = True

        self.Send(commandstring.encode())
    ...
    def __ReceiveData(self, interface, data):
        ...
        #print('Screen controller reply data: {}'.format(data))
