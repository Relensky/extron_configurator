...
DEVICE_INFO = {
    ...
    "models": ["VPL-PHZ60", "VPL-P620HZ", "VPL-PHZ50", "VPL-P520HZ"],
    "connection": {
        "com_type": "Network",
        "protocol": "TCP",
        "net_port": 53595,
...
class DeviceClass:
    def __init__(self):
        self.WarmUpTime = 15.0
        self.CoolDownTime = 15.0
        self.Unidirectional = 'False'
        self.connectionCounter = 3
    ...
    def UpdateLampUsage(self, value, qualifier):
        # VPL-PHZ60 is Laser. 'timer ?' (Lamp) often returns err_cmd.
        # Try 'light_timer ?' or just suppress the error to clean up logs.
        LampUsageCmdString = 'timer ?\r\n' 
        res = self.__UpdateHelper('LampUsage', LampUsageCmdString, value, qualifier)
        if res:
            try:
                # Check if device returned error, if so, ignore it for this specific command
                if 'err_cmd' in res.lower():
                    return 
                    
                value = int(res.strip().strip('"'))
                self.WriteStatus('LampUsage', value, qualifier)
            except (ValueError, IndexError):
                # Suppress error log for Timer to avoid spamming 'Invalid response'
                pass
    ...
    def UpdatePower(self, value, qualifier):
        ValueStateValues = {
            ...
            '"off"': 'Off',
            '"standby"': 'Off',
            '"warming"': 'On', # Add missing intermediate states
            '"cooling"': 'Off'
        }
        
        # Try standard ADCP status first
        PowerCmdString = 'power_status ?\r\n'
        res = self.__UpdateHelper('Power', PowerCmdString, value, qualifier)
        
        # If power_status fails (err_cmd), try the simpler 'power ?' command
        if res and 'err_cmd' in res.lower():
            PowerCmdString = 'power ?\r\n'
            res = self.__UpdateHelper('Power', PowerCmdString, value, qualifier)
        ...
        if res:
            try:
                clean_res = res.strip()
                # Handle cases where response might be just "on" or "off" without quotes
                if clean_res in ValueStateValues:
                    value = ValueStateValues[clean_res]
                elif '"{}"'.format(clean_res) in ValueStateValues:
                    value = ValueStateValues['"{}"'.format(clean_res)]
                else:
                    # Fallback for simple on/off
                    if 'on' in clean_res.lower(): value = 'On'
                    elif 'off' in clean_res.lower(): value = 'Off'
                    else: return
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True':
            ...
            res = self.SendAndWait(commandstring.encode(), self.DefaultResponseTimeout, deliTag=b'\r\n')
            if not res:
    ...
    # Change to update helper to manage connection status to False on boot
        ...
        if self.__is_authenticated:
            if self.Unidirectional == 'True':
                ...
                if self.counter > self.connectionCounter:
                    # Always fire OnDisconnected, even if we were never fully connected.
                    # This ensures the 'Initializing' status is cleared to 'Disconnected'.
                    self.OnDisconnected()
                    return ''
                res = self.SendAndWait(commandstring.encode(), self.DefaultResponseTimeout, deliTag=b'\r\n')
                ...
                if not res:
                    ...
                    # Only mark connected if it actually got a response (res is not None)
    ...
    def WriteStatus(self, command, value, qualifier=None):
        # Only reset counter and force Connect if this is NOT a connection status update.
        # This prevents 'Disconnected' events from accidentally triggering 'Connected'.
