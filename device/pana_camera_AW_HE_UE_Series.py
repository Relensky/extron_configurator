...
from modules.project.logger import Logger
import re
import base64
import urllib.error
...
class DeviceSerialClass:
    ...
    def MissingCredentialsLog(self, credential_type):
        if isinstance(self, EthernetClientInterface):
            ...
            return 
        self.logger.warning("{0} module received a request from the device for a {1}, "
                "but device{1} was not provided.\n Please provide a device{1} "
                "and attempt again.\n Ex: dvInterface.device{1} = '{1}'\n Please "
                "review the communication sheet.\n {2}"
                .format(__name__, credential_type, port_info))
...
class DeviceHTTPClass:
    def __init__(self, ipAddress, port, deviceUsername, devicePassword):
        ...
        self.Commands = {
            ...
            'ResetZoom': { 'Status': {}},
            'SceneFileControl': { 'Status': {}, 'AllowedValues': ['Manual 1', 'Manual 2', 'Manual 3', 'Full Auto']},
            'Tally': { 'Status': {}},
            'TallyInput': { 'Status': {}},
        ...
        self.Power = re.compile('p([0-3fnFN])')
    ...
    def SetAutoFocus(self, value, qualifier):
        FocusValues = {
            'Auto':     '1',
            'Manual':   '0',
            'On':       '1',  # Added to support Camera Manager
            'Off':      '0'   # Added to support Camera Manager
        }
    ...
    def SetIris(self, value, qualifier):
        # Initialize an internal variable to track the Iris level (1 to 99)
        # start it at 50 (middle) the very first time the button is pressed.
        if not hasattr(self, '_current_iris'):
            self._current_iris = 50  
            
        # Define how big of a jump you want per click
        step_amount = 5  
        
        # Increment or decrement the tracked value based on the button press
        if value == 'Up':
            # Add to the current iris, but cap it at 99 (Max)
            self._current_iris = min(99, self._current_iris + step_amount)
        elif value == 'Down':
            # Subtract from the current iris, but don't let it go below 1 (Min)
            self._current_iris = max(1, self._current_iris - step_amount)
            
        # Format and send the absolute position command (cmd=#IXX)
        data = 'cmd=%23I{0:02d}&res=1'.format(self._current_iris)
        self.__SetHelper('Iris', value, qualifier, url='', data=data)
    ...
    def SetBackLight(self, value, qualifier):
        BackLightValues = { 'Off': '0', 'On': '1' }
        if value in BackLightValues:
            data = 'cmd=OSE:73:{0}&res=1'.format(BackLightValues[value])
            self.__SetHelper('BackLight', value, qualifier, url='', data=data)

    def SetPresetRecall(self, value, qualifier):
        preset_idx = int(value) - 1
        data = 'cmd=%23R{0:02d}&res=1'.format(preset_idx)
        self.__SetHelper('PresetRecall', value, qualifier, url='', data=data)

    def SetPresetSave(self, value, qualifier):
        preset_idx = int(value) - 1
        data = 'cmd=%23M{0:02d}&res=1'.format(preset_idx)
        self.__SetHelper('PresetSave', value, qualifier, url='', data=data)

    def SetGain(self, value, qualifier):
        if value == 'Up':
            data = 'cmd=OGU:11&res=1' 
        elif value == 'Down':
            data = 'cmd=OGU:08&res=1' 
        else:
            data = 'cmd=OGU:80&res=1' 
        self.__SetHelper('Gain', value, qualifier, url='', data=data)

    def SetShutter(self, value, qualifier):
        if value == 'Up':
            data = 'cmd=OSH:6&res=1' 
        elif value == 'Down':
            data = 'cmd=OSH:0&res=1' 
        self.__SetHelper('Shutter', value, qualifier, url='', data=data)

    def SetAutoExposure(self, value, qualifier):    
        if value in ["Full Auto", "Bright"]:
            data = 'cmd=XSF:4&res=1'  # Scene File Full Auto
        else:
            data = 'cmd=ORS:1&res=1'  # Auto Iris
        self.__SetHelper('AutoExposure', value, qualifier, url='', data=data)
    ...
    def SetFocus(self, value, qualifier):
        FocusConstraints = {'Min': 1, 'Max': 49}
        
        # Look for 'Focus Speed' instead of 'Speed'
        Speed = int(qualifier.get('Focus Speed', 24))
    ...
    def SetPanTilt(self, value, qualifier):
        PanTiltConstraints = {'Min': 1, 'Max': 49}
        # Fallback to 24 if the key isn't found, handle both keys
        Speed = int(qualifier.get('Pan Speed', qualifier.get('Tilt Speed', 24)))
    ...
    def SetPower(self, value, qualifier):
        ...
        # Define the feedback mapping locally for parsing the response
        PowerStateNames = {
            '0': 'Off',
            '1': 'On',
            '2': 'On',
        ...
        # Capture the result of the SetHelper
        res = self.__SetHelper('Power', value, qualifier, url='', data=data)

        # Process the immediate response from the camera to update UI/GVE
        if res:
            try:
                mGroup = self.Power.search(res)
                if mGroup is not None:
                    PowerValue = mGroup.group(1)
                    if PowerValue in PowerStateNames:
                        new_state = PowerStateNames[PowerValue]
                        self.logger.info("SetPower Response: Camera reported {}".format(new_state))
                        self.WriteStatus('Power', new_state, None)
            except Exception as e:
                self.logger.error("Error parsing SetPower response: {}".format(e))
    ...
    def SetZoom(self, value, qualifier):
        ZoomConstraints = {'Min': 1, 'Max': 49}
        
        # Look for 'Zoom Speed' instead of 'Speed'
        Speed = int(qualifier.get('Zoom Speed', 24))
    ...
    def __SetHelper(self, command, value, qualifier, url='', data=None):
        ...
        # --- LOGGING INCOMING SET COMMAND ---
        self.logger.info("Camera HTTP SEND (Set {}): {}".format(command, url))
        ...
        try:
            ...
            if res.status not in (200, 202):
                ...
                # --- LOGGING CAMERA RESPONSE ---
                if res:
                    self.logger.info("Camera HTTP RECV (Set {}): {}".format(command, res))
        return res
    ...
    def __UpdateHelper(self, command, value, qualifier, url='', data=None):
        ...
        # --- LOGGING OUTGOING UPDATE/QUERY COMMAND ---
        self.logger.info("Camera HTTP SEND (Update {}): {}".format(command, url))
        ...
        try:
            ...
            if res.status not in (200, 202):
                ...
                # --- LOGGING CAMERA RESPONSE ---
                if res:
                    self.logger.info("Camera HTTP RECV (Update {}): {}".format(command, res))
        return res
