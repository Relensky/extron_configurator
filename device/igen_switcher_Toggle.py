...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'AutoSwitch': { 'Status': {}, 'AllowedValues': ['On', 'Off']},
            'ButtonLock': { 'Status': {}, 'AllowedValues': ['On', 'Off']},
            'DeviceCurrent': {'Parameters': ['Device'], 'Status': {}},
            'FirmwareVersion': { 'Status': {}},
            'GPINMode': { 'Status': {}, 'AllowedValues': ['Pulse', 'Level']},
            'HubPower': {'Parameters': ['Hub'], 'Status': {}},
            'Input': { 'Status': {}, 'AllowedValues': ['1', '2', 'Off']},
    ...
    def UpdateDeviceCurrent(self, value, qualifier):
        ...
        # Only query valid ports (1, 2, 3)
        if device in ['1', '2', '3']:
            DeviceCurrentCmdString = 'GDI {}\r'.format(device)
            res = self.__UpdateHelper('DeviceCurrent', DeviceCurrentCmdString, value, qualifier)
            
            if res:
                # The device returns dirty data like: "GDI 1\r\n0.01A\n\rACK\n\r"
                # We use Regex to find the number before the 'A'
                match = re.search(r'(\d+\.\d+)A', res)
                
                if match:
                    try:
                        # Extract the value (e.g., 0.01)
                        current_val = float(match.group(1))
                        self.WriteStatus('DeviceCurrent', current_val, qualifier)
                    except (ValueError, IndexError):
                        self.Error(['Device Current: Parsing error on value: {}'.format(match.group(1))])
                elif "ACK" in res:
                    # If we get an ACK but no current value (device might be sleeping), just mark connected
                    # We write the existing value or 0.0 just to keep the connection alive
                    self.WriteStatus('ConnectionStatus', 'Connected') 
                else:
                    self.Error(['Device Current: Invalid response pattern'])
        else:
