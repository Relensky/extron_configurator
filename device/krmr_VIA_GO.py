...
class DeviceClass:
    def __init__(self):
        ...
        self.commandhead = "<P><UN>{0}</UN><Pwd>{1}</Pwd>".format(
            self._deviceUsername, self._devicePassword
        )
        ...
        if self.Unidirectional == "False":
            self.AddMatchString(
                re.compile(b"Login Successful."), self.__MatchLoginSuccess, None
            )
            self.AddMatchString(
                re.compile(b"Error(1[1234])"), self.__MatchLoginFail, None
            )

        self.update_delirex = {
            "DisplayStatus": re.compile(
                b"DisplayStatus\|Get\|(?:Not ?Presenting|Presenting|Waiting|UsrNotExist)\r"
            ),
            "IPAddress": re.compile(b"IP:.*?\|"),
            "MACAddress": re.compile(
                b".*?\r"
            ),  # Generic match to capture whatever it sends
    ...
    def __MatchLoginSuccess(self, match, tag):
        ...
        self.command = (
            "<P><UN>{0}</UN><Pwd></Pwd>".format(self._deviceUsername)
            + self.commandbody
            + self.commandtail
        )
    ...
    def UpdateDisplayStatus(self, value, qualifier):
        ...
        if user:
            DisplayStatusCmdString = self.command.format(
                "DisplayStatus", "Get", user, ""
            )
            res = self.__UpdateHelper(
                "DisplayStatus", DisplayStatusCmdString, value, qualifier
            )
            if res:
                try:
    ...
    def SetDoNotDisturb(self, value, qualifier):
        ...
        ValueStateValues = {"On": "Set", "Off": "UnSet"}

        if user and value in ValueStateValues:
            DoNotDisturbCmdString = self.command.format(
                "DND", ValueStateValues[value], user, ""
            )
    ...
    def SetPresentationMode(self, value, qualifier):
        ...
        if value in ValueStateValues:
            PresentationModeCmdString = self.command.format(
                "PrsMode", "Set", ValueStateValues[value], ""
            )
            self.__SetHelper(
                "PresentationMode", PresentationModeCmdString, value, qualifier
            )
        else:
    ...
    def SetRestartApacheServer(self, value, qualifier):
        ...
        self.__SetHelper(
            "RestartApacheServer", RestartApacheServerCmdString, value, qualifier
        )
    ...
    def SetWakeUp(self, value, qualifier):
        ...
        ValueStateValues = {"On": "0", "Off": "1"}

        if value in ValueStateValues:
            WakeUpCmdString = self.command.format(
                "WakeUp", ValueStateValues[value], "", ""
            )
    ...
    def SetWifiGuestMode(self, value, qualifier):
        ...
        ValueStateValues = {"Start": "1", "Stop": "0"}

        if value in ValueStateValues:
            WifiGuestModeCmdString = self.command.format(
                "WifiGuestMode", ValueStateValues[value], "", ""
            )
    ...
    def UpdateWifiGuestMode(self, value, qualifier):
        ...
        ValueStateValues = {"1": "Start", "0": "Stop"}

        WifiGuestModeCmdString = self.command.format("WifiGuestMode", "Status", "", "")
        res = self.__UpdateHelper(
            "WifiGuestMode", WifiGuestModeCmdString, value, qualifier
        )
        if res:
            try:
                value = ValueStateValues[res.strip().split("|")[-1]]
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        if "Error" in response:
            self.Error(
                ["An error occurred: {}: {}.".format(sourceCmdName, response.strip())]
            )
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == "True":
            ...
            res = self.SendAndWait(
                commandstring, self.DefaultResponseTimeout, deliTag=b"\r"
            )
            if not res:
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        print(
            "VIA Update: Command={}, AuthState={}".format(command, self.authenticated)
        )
        if self.authenticated:
            if self.Unidirectional == "True":
                ...
                res = self.SendAndWait(
                    ...
                    deliRex=self.update_delirex[command],
                )
                if not res:
                    return ""
                else:
