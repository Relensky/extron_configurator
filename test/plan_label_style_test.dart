...
/// lands on a wall. Every label is printed on a plate; this is what color that
...
/// same drawing.
void main() {
  test('a sheet nobody has recolored follows the drawing it is on', () {
    ...
    // Which is what the two resolvers mean by an unset color.
    expect(
  ...
  test('a color that IS set wins over the drawing default', () {
  ...
  test('the colors survive a save and a reload of the sheet', () {
    const sheet = FloorPlan(id: 'PLAN_1', name: 'Level 1');
    final recolored = sheet
        .withLabelStyle(
          PlanTextKind.wiring,
    ...
    final back = FloorPlan.fromJson(recolored.toJson());
  ...
  test('a sheet put back to standard saves as one that was never touched', () {
    const sheet = FloorPlan(id: 'PLAN_1', name: 'Level 1');
    final recolored = sheet.withLabelStyle(
      PlanTextKind.callout,
      const PlanLabelStyle(background: 0xFF123456),
    );
    final reset = recolored.withLabelStyle(
      PlanTextKind.callout,
  ...
  test('the provider recolors only the sheet it was asked about', () {
