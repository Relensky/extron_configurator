...
def build():
    ...
    gear_poly = gear_points(cx, gear_cy, r_tip, r_root, 12)
    face_box = [cx - r_hole, face_cy - r_hole, cx + r_hole, face_cy + r_hole]
    ...
    lay(img, SHADE_COOL, clip(gear_mask, vramp(px(620), px(890), 76, 0)))
    lay(img, SHADE_COOL, clip(gear_mask, vramp(px(900), px(1020), 0, 42)))
    d.polygon(gear_poly, outline=CHARCOAL, width=w)
    ...
    lay(img, SHADE_COOL, clip(face_mask, vramp(px(610), px(800), 58, 0)))
