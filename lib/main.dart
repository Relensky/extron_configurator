...
class RoomConfigApp extends StatelessWidget {
  ...
  /// its own accent swatch.
  static ThemeData themeFor(
      String style, bool isDark, String classicColor, String aurisColor) {
    switch (style) {
      case 'auris':
      ...
      default:
        final scheme =
            FlexSchemeColor.from(primary: parseHexColor(classicColor));
        return isDark
...
/// Swatch grid for a theme accent color. Used for both the Classic and
/// Auris styles in App Config (and Classic in the first-run setup dialog);
/// tapping a swatch persists it immediately and the theme rebuilds live.
class AccentColorPicker extends StatelessWidget {
  /// Which provider setting the picker writes: 'classicColor' | 'aurisColor'.
  final String settingKey;
  final List<Color> swatches;

  const AccentColorPicker(
      {Key? key, required this.settingKey, required this.swatches})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    final provider = context.watch<AppStateProvider>();
    final Color current = RoomConfigApp.parseHexColor(settingKey == 'aurisColor'
        ? provider.aurisColor
