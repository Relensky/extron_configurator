  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      ...
      content: Text(
        ...
        '${result.nodesAdded == 0 ? '' : ', ${result.nodesAdded} source'
            '${result.nodesAdded == 1 ? '' : 's'} added'}'
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              'the connector each display\'s own "input" names.'
              '${plan.newNodes.isEmpty ? '' : ' ${plan.newNodes.length} '
                  'source${plan.newNodes.length == 1 ? '' : 's'} the config '
                  'names but has no device block for — the PC, the doc cam, '
                  'the laptop plates — will be added to the canvas.'}'
